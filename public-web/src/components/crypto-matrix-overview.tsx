...
import type { CryptoMatrixAsset, CryptoMatrixCell, CryptoMatrixData, CryptoMatrixView } from "@/lib/types";
...
function buildCellMap(cells: CryptoMatrixCell[]) {
  ...
  for (const cell of cells) map.set(cellKey(cell.assetKey, cell.accountName), cell);
  return map;
}
  ...
  return (
    <span className="pointer-events-auto absolute left-1/2 top-5 z-30 hidden w-[min(360px,calc(100vw-48px))] -translate-x-1/2 rounded-2xl border border-[color:var(--border-strong)] bg-[color:var(--paper)] p-4 text-left shadow-[0_18px_50px_rgba(44,33,22,0.18)] group-hover:block group-focus-within:block">
      <span className="mb-2 flex flex-wrap items-center gap-2">
        <Badge variant={view.direction === "positive" ? "positive" : view.direction === "negative" ? "danger" : "neutral"} className="normal-case">
          {view.date}
        </Badge>
      ...
      {view.note_urls.length ? (
        <span className="mt-3 flex flex-wrap gap-1.5">
          {view.note_urls.map((url, index) => (
            <span key={`${url}-${index}`} className="inline-flex items-center gap-1 rounded-full border border-[color:var(--border-strong)] px-2 py-1 text-[11px] text-[color:var(--muted-ink)]">
              来源 {index + 1}
...
export function CryptoMatrixOverview() {
  ...
  useEffect(() => {
    ...
    getVisibleCryptoMatrix(supabase, endDate)
      .then((nextData) => {
        if (cancelled) return;
        setData(nextData);
        setError(null);
  ...
  const totalViews = useMemo(() => (data?.cells ?? []).reduce((sum, cell) => sum + cell.views.length, 0), [data]);
        ...
        <CardHeader className="gap-4 lg:py-5">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <CardDescription>绿点为正向，红点为负向，灰点为转发、提及、公告或数据播报等弱信号。</CardDescription>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button type="button" variant="secondary" disabled={!data?.previousEndDate} onClick={() => navigate(data?.previousEndDate ?? null)}>
                <ArrowLeft className="h-4 w-4" />
                上一周
              </Button>
              <Button type="button" variant="secondary" disabled={!data?.nextEndDate} onClick={() => navigate(data?.nextEndDate ?? null)}>
                下一周
      ...
      <div className="grid gap-4 lg:min-h-0 lg:flex-1 lg:grid-rows-[auto_minmax(0,1fr)]">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--paper)] px-4 py-3">
            ...
            <p className="mt-1 text-sm font-semibold">{formatCount(data?.assets.length ?? 0)}</p>
          </div>
          <div className="rounded-2xl border border-[color:var(--border)] bg-[color:var(--paper)] px-4 py-3">
        ...
        <Card className="min-h-[420px] overflow-hidden">
          <CardContent className="h-full p-0">
            ...
            {!matrixLoading && data && data.assets.length && data.authors.length ? (
                  ...
                  <thead>
                    <tr>
                      ...
                      {data.authors.map((author) => (
                        <th key={author.accountName} className="sticky top-0 z-20 min-w-[148px] max-w-[148px] border-b border-r border-[color:var(--border)] bg-[color:var(--paper-strong)] px-3 py-3 align-bottom">
                          <Link href={`/crypto/feed?q=${encodeURIComponent(author.accountName)}`} className="block truncate text-[12px] font-semibold normal-case tracking-normal text-[color:var(--ink)] underline-offset-4 hover:text-[color:var(--accent-strong)] hover:underline" title={author.authorNickname || author.accountName}>
                    ...
                    {data.assets.map((asset) => (
                      <tr key={asset.assetKey}>
                        <th className="sticky left-0 z-10 min-w-[220px] border-b border-r border-[color:var(--border)] bg-[color:var(--paper)] px-4 py-3 text-left align-top">
                          <Link href={`/crypto/assets?asset=${encodeURIComponent(asset.assetKey)}`} className="block text-sm font-semibold normal-case tracking-normal text-[color:var(--ink)] underline-offset-4 hover:text-[color:var(--accent-strong)] hover:underline">
                            {asset.displayName}
                          </Link>
                        ...
                        {data.authors.map((author) => {
                          ...
                          return (
                            <td key={`${asset.assetKey}-${author.accountName}`} className="min-w-[148px] max-w-[148px] border-b border-r border-[color:var(--border)] bg-[color:var(--panel)]/70 px-3 py-3 align-top">
                              {cell?.views.length ? (
                                <div className="flex flex-wrap gap-1.5">
                                  {cell.views.map((view, index) => (
                                    <SignalDot key={`${asset.assetKey}-${author.accountName}-${view.date}-${index}`} asset={asset} view={view} />
                                  ))}
                                </div>
                              ) : (
            ...
            {!matrixLoading && data && (!data.assets.length || !data.authors.length) ? (
              <div className="m-4 empty">{profile ? "这个窗口没有 crypto 标的信号" : "暂无公开预览数据"}</div>
            ) : null}
          </CardContent>
        </Card>
      </div>
