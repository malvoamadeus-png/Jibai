    ...
    return [
        {
            ...
            "content": (
                ...
                "每条观点必须包含字段："
                "entity_type, entity_name, entity_code_or_name, stance, direction, judgment_type, conviction, evidence_type, logic, evidence, time_horizon。"
                ...
                "如果对象只是上市公司里的业务部门、产品线、供应链角色，能明确归属上市母公司时归到母公司股票；不能明确归属时才用 theme 或 other。"
                "stance 只能是 strong_bullish, bullish, neutral, bearish, strong_bearish, mixed, mention_only, unknown 之一。"
                "direction 表示信息对该对象的方向，只能是 positive, negative, neutral, mixed, unknown 之一。"
                "judgment_type 表示作者判断参与度，只能是 direct, implied, factual_only, quoted, mention_only, unknown 之一。"
                "direct 表示作者直接给出看多、看空、买卖、仓位、目标价等判断；"
                "implied 表示作者没有直接喊方向，但通过选择和组织事实构建了明显正向或负向叙事；"
                "factual_only 表示只是行情、财报、数据、新闻等事实陈述，不能确认作者本人判断；"
                "quoted 表示主要是在转述公司管理层、机构或他人的判断；mention_only 表示仅提及。"
                ...
                "stance 是兼容旧界面的派生字段：direction 为 positive 时用 bullish/strong_bullish，negative 时用 bearish/strong_bearish，neutral 时用 neutral，mixed 时用 mixed，仅提及时用 mention_only。"
                "time_horizon 只能是 short_term, medium_term, long_term, unspecified 之一。"
                "不要把价格上涨、财报超预期、管理层表述自动判成作者直接看多；如果作者只是列出事实，用 factual_only；如果作者借这些事实组织成明确叙事，用 implied。"
                "如果只是提到对象但没有有效方向或判断，judgment_type 和 stance 都用 mention_only。"
