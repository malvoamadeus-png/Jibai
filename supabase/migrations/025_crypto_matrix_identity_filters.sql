alter table public.crypto_asset_narrative_briefs
  add column if not exists identity_status text
    check (identity_status in ('anchored', 'fuzzy', 'ambiguous'));

update public.crypto_asset_narrative_briefs
set identity_status = case
  when ca_resolution_status in ('resolved', 'existing_identifier') then 'anchored'
  when status = 'succeeded' then 'fuzzy'
  else null
end
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'identity_status', cab.identity_status,
