create table if not exists public.crypto_asset_narrative_briefs (
  id uuid primary key default gen_random_uuid(),
  asset_key text not null unique references public.crypto_entities(asset_key) on delete cascade,
  status text not null default 'failed'
    check (status in ('succeeded', 'failed', 'skipped')),
  ca_resolution_status text not null default 'unresolved'
    check (ca_resolution_status in ('resolved', 'unresolved', 'existing_identifier')),
  contract_address text,
  chain_index text,
  resolved_by text not null default '',
  summary_text text not null default '',
  candidate_contracts_json jsonb not null default '[]'::jsonb,
  ...
  model_name text,
  prompt_version text not null default 'crypto_asset_narrative_v1',
...
create index if not exists crypto_asset_narrative_briefs_status_idx
  on public.crypto_asset_narrative_briefs(status, updated_at desc);

create index if not exists crypto_asset_narrative_briefs_seen_idx
  on public.crypto_asset_narrative_briefs(latest_seen_date desc, updated_at desc);

drop trigger if exists set_crypto_asset_narrative_briefs_updated_at on public.crypto_asset_narrative_briefs;
create trigger set_crypto_asset_narrative_briefs_updated_at before update on public.crypto_asset_narrative_briefs
for each row execute function public.set_updated_at();

revoke all on table public.crypto_asset_narrative_briefs from public;
...
begin
  ...
  from (
    ...
    where lower(regexp_replace(coalesce(view_item.value ->> 'account_name', view_item.value ->> 'author_name', view_item.value ->> 'username', ''), '^@', '')) <> ''
      and public.crypto_view_is_asset_candidate(view_item.value)
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'asset_key', ar.asset_key,
        'display_name', ar.display_name,
        'ticker', ar.symbol,
        'market', ar.market,
        'mention_count', ar.mention_count,
        'latest_date', ar.latest_date::text,
        'summary', case
          when cab.status = 'succeeded' and nullif(cab.summary_text, '') is not null then cab.summary_text
          else ''
        end,
        'summary_status', coalesce(cab.status, 'missing'),
        'summary_updated_at', case when cab.updated_at is null then null else cab.updated_at::text end
      )
      order by ar.latest_date desc, ar.mention_count desc, ar.display_name asc
    ),
    '[]'::jsonb
  )
  into assets_payload
  from asset_rows ar
  left join public.crypto_asset_narrative_briefs cab on cab.asset_key = ar.asset_key;
    ...
    select
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'entity_identifier_type', coalesce(view_value ->> 'entity_identifier_type', 'unknown'),
          'raw_identifiers', coalesce(view_value -> 'raw_identifiers', '[]'::jsonb),
          'normalized_status', coalesce(view_value ->> 'normalized_status', 'canonical'),
          'source_signal_level', coalesce(view_value ->> 'source_signal_level', 'strong'),
