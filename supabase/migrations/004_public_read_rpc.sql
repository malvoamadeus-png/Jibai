-- Controlled public reads for the browser-only public-web app.
-- Anonymous users can browse approved accounts and see one lightweight real
-- preview per timeline surface. Authenticated users are scoped to their
-- subscriptions; admins can read all approved analysis data.
...
as $$
  ...
  order by coalesce(a.approved_at, a.updated_at, a.created_at) desc, a.username::text asc
...
begin
  ...
  eligible_days as (
    ...
    where exists (
      ...
      where coalesce(viewpoint.value ->> 'stance', '') <> 'mention_only'
    )
  )
  select
    d.id as account_id,
  ...
  if entity_type_arg = 'theme' then
    ...
    expanded as (
      select
        ...
        te.display_name::text as display_value,
        null::text as ticker_value,
        null::text as market_value,
        tdv.date_key::text as date_value,
        tdv.updated_at,
        lower(regexp_replace(coalesce(view_item.value ->> 'account_name', view_item.value ->> 'author_name', view_item.value ->> 'username', ''), '^@', '')) as author_name
