create or replace function public.crypto_view_is_asset_candidate(view_value jsonb)
returns boolean
language sql
immutable
as $$
  select case
    when lower(coalesce(view_value -> 'metadata' ->> 'asset_candidate', 'true')) in ('false', '0', 'no') then false
    else true
  end;
$$;
