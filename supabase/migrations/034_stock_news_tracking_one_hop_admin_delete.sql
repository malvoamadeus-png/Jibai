...
create or replace function public.delete_stock_news_tracking_stock(
  stock_row_id_arg uuid
)
returns jsonb
language plpgsql
security definer
...
begin
  if current_uid is null or not public.is_current_user_admin() then
    raise exception 'Only admins can delete stock news tracking rows' using errcode = '42501';
  end if;

  delete from public.stock_news_tracking_stocks
  where id = stock_row_id_arg
  returning id, tracking_id, security_key, display_name
  into deleted_row;

  if not found then
    raise exception 'tracking stock row not found' using errcode = '02000';
  end if;

  return jsonb_build_object(
    'id', deleted_row.id,
    'tracking_id', deleted_row.tracking_id,
    'security_key', deleted_row.security_key,
...
revoke all on function public.delete_stock_news_tracking_stock(uuid) from public;
grant execute on function public.delete_stock_news_tracking_stock(uuid) to authenticated;
