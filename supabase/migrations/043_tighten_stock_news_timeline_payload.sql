create or replace function public.slim_stock_news_linked_entity(entity_value jsonb)
returns jsonb
language sql
immutable
as $$
  select jsonb_strip_nulls(jsonb_build_object(
    ...
    'entity_key', entity_value ->> 'entity_key',
    'entity_name', entity_value ->> 'entity_name',
    'entity_code_or_name', entity_value ->> 'entity_code_or_name'
  ));
$$;
...
as $$
  select jsonb_strip_nulls(jsonb_build_object(
    ...
    'linked_entities', coalesce(
      (
        select jsonb_agg(public.slim_stock_news_linked_entity(entity_item.value) order by entity_item.ordinality)
        from jsonb_array_elements(coalesce(event_value -> 'linked_entities', '[]'::jsonb)) with ordinality as entity_item(value, ordinality)
      ),
      '[]'::jsonb
    ),
...
create or replace function public.get_visible_stock_news_timeline(
  ...
  page_size_arg integer default 5
)
returns jsonb
language plpgsql
security definer
...
begin
  safe_page_size := case
    ...
    else least(greatest(coalesce(page_size_arg, 5), 1), 10)
  end;
...
revoke all on function public.slim_stock_news_linked_entity(jsonb) from public;
grant execute on function public.slim_stock_news_linked_entity(jsonb) to anon, authenticated;
