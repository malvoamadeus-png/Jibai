...
`/stocks/news/tracking` 删除整条追踪新闻、批量删除多条映射股票，或删除单条新闻下不匹配的映射股票。每天北京时间 08:00
和 20:00 刷新映射股票行情，涨幅锚点使用新闻 `event_date` 对应的可用交易日收盘价，不使用 AI 后续分析当天；刷新任务也会顺带修正早期误写的锚点日期。价格刷新按 `security_key` 读取行情缓存并回填缺失的 `security_id`，单条映射股票失败不会中断整批刷新。
价格刷新按“最久未刷新优先”分批轮转，避免新追踪新闻长期挤占旧新闻的补价机会。
