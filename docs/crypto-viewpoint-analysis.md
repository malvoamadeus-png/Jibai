...
- `entity_kind`：优先使用 `project`、`token`、`org_or_fund`、`account`、`theme_or_generic` 或 `unknown`。
- `canonical_name_hint`：当项目名、项目账号、symbol 或 meme ticker 明显指向同一实体时，给出更稳定的规范名称提示。
- `investable_score`、`specificity_score`、`entityness_score`：`0` 到 `1` 的浮点分数，用于后续 resolver 和展示层判断。
- `is_generic_term`：布尔值。赛道词、主题词、泛项目描述或泛概念应标记为 `true`。
...
- 未命中 `crypto_aliases.json` 时，系统会先生成中性的 provisional key：
  `project_name` / `project_account` 优先归到 `proj:<slug>`，`symbol` / `meme_ticker` 优先归到 `tick:<slug>`；后续再由 alias 配置或重分析归并到 canonical `asset_key`。
- 合约地址和 Solana 地址仍优先按精确地址归一化，不走文本 provisional key。
...
从 2026-05-21 起，`content_viewpoints.metadata_json` 会额外保存 resolver 线索和展示过滤线索，例如：
...
`/crypto` 首页的“公开预览 / 最近更新”使用轻量 RPC `get_home_feed_preview(domain='crypto')`。这个接口只返回首页卡片实际需要的作者、日期、内容数、信号数和更新时间，不再复用完整作者时间线读取链路。
...
- 不展示作者当天总括总结句。
- 作者时间线和资产时间线都只把 `asset_candidate=true` 的实体计入“标的”展示；被识别为 `org_or_fund`、`account`、`theme_or_generic` 或明显泛词的 mention 仍保留在原始分析记录里，供后续复核和归并，但默认不进入资产视图。
...
- `/crypto/assets/overview` 现已支持和 `/stocks/overview` 一致的时间粒度切换：默认按周展示最近 7 个自然日，也可以切到按日只查看单个自然日。
- 按日和按周两种模式都以前端当前可见范围里的最新日期作为默认结束日，并支持向前、向后翻页以及回到最新。
- 当前窗口里完全没有信号的作者列和标的行不会渲染。
