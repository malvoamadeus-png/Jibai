...
| 按股票（一览表） | `/stocks/overview` | 查看股票 x 作者观点矩阵，支持按周或按日切换，也支持前端“紧凑显示”把每只股票压缩成只展示自己实际出现的作者列，并自动隐藏当前窗口里完全空白的行列。 |
...
首页统计卡片也不再依赖整份账号列表；当前改为通过 `get_home_stats` 单独返回已审批账号数和当前用户订阅数。

这样可以减少 Supabase 调用次数和返回体积，同时保持和 `/feed` 相同的数据新鲜度，因为它仍然直接读取最新的 `author_daily_summaries` 和账号/订阅关系表。
...
详情页里的 K 线 `markers` 现在只返回图表和当日侧栏真正需要的轻量字段，例如作者、方向、信号类型和逻辑；完整 `author_views` 只保留在时间线 `rows` 里，避免同一批观点在同一次 RPC 响应里重复传输两份。
...
- 普通模式：使用全局作者表头，按固定矩阵列展示所有当前窗口内出现过的作者。
- 紧凑显示：每只股票只展示自己实际出现过的作者列，不再强制补齐全局作者空列。作者列顺序仍沿用当前窗口里的全局作者顺序过滤后子集，作者标签使用短简称，完整作者信息通过 hover 或标题提示补充。

## 本地导出每日作者观点

为了给 X 和小红书制作日更图片素材，本地后端提供了一个直接导出 CSV 的命令：
...
- 默认导出本地 SQLite 中最新一个存在有效股票观点的 `date_key`。
- 可用 `--date YYYY-MM-DD` 指定自然日，`--platform x` 只导出单个平台。
- 默认输出到 `data/runtime/exports/daily-author-viewpoints-YYYY-MM-DD.csv`。
- 每一行代表“某作者在当天对某个股票的一条聚合观点”。
- 导出字段为：`日期`、`作者`、`作者账号`、`标的名称`、`标的代码/简称`、`多空`、`强烈程度`、`逻辑`。
- 不导出 `evidence`，也不导出纯提及、无方向或非股票对象。
