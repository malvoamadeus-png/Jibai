...
from .models import BloggerPost
...
def validate_username(raw: str) -> str:
    username = normalize_account(raw)
...
def _is_pure_retweet(payload: dict[str, Any]) -> bool:
...
def post_from_status(payload: dict[str, Any], username: str) -> BloggerPost | None:
    author = payload.get("author") if isinstance(payload.get("author"), dict) else {}
    screen_name = str(author.get("screen_name") or "").strip().lstrip("@")
    if screen_name.casefold() != username.casefold():
        return None
    if _is_pure_retweet(payload):
        return None
...
def fetch_posts(
    *,
    ...
    existing_posts: list[BloggerPost] | None = None,
) -> tuple[list[BloggerPost], list[dict[str, Any]], dict[str, Any]]:
    seen_ids = {post.tweet_id for post in existing_posts or []}
    posts: list[BloggerPost] = list(existing_posts or [])
